...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["gambar"]["name"])) { // Periksa apakah file gambar baru dipilih
        ...
        if (in_array($file_ext, $extensions) === false) {
            ...
            if (move_uploaded_file($file_tmp, $target_file)) {
                ...
                $sql_update = "UPDATE user SET gambar='$new_file_name' WHERE userID='$userID'";
        ...
        $sql_update = "UPDATE user SET namaLengkap='$namaUserBaru', notelp='$notelpBaru', penjelasan='$deskripsi', alamat= '$alamatBaru' WHERE userID='$userID'";
        if ($conn->query($sql_update) === true) {
    ...
    $password_query = "SELECT password FROM user WHERE userID = '$userID'";
    ...
    if ($password_result->num_rows > 0) {
        ...
        if (password_verify($passwordOld, $hashed_password)) {
            ...
            $update_password_query = "UPDATE user SET password='$hashed_new_password' WHERE userID='$userID'";
...
<div class="main-content">
    <section class="section">
        ...
        <div class="section-body">
            <h2 class="section-title">Hi, <?= $namaUser ?> </h2>
