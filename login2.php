...
function getUserByUsername($username, $conn) {
    $query = "SELECT userID, password, namaUser, status, hakAkses FROM user2 WHERE username = ?";
            ...
            if (mysqli_num_rows($result) == 1) {
                ...
                if (password_verify($password, $row['password'])) {
                    if ($row['status'] == "Aktif") {
                        ...
                        // Pengecekan hak akses dan pengalihan halaman sesuai dengan hak akses
                        if ($_SESSION['hakAkses'] == "Admin") {
                            // Jika pengguna adalah admin, arahkan ke halaman admin
                            header("Location: admin/");
                            exit();
                        } elseif ($_SESSION['hakAkses'] == "Pustakawan") {
                            // Jika pengguna adalah pustakawan, arahkan ke halaman perpustakaan
                            header("Location: perpustakaan/");
                            exit();
                        } else {
                            // Jika hak akses tidak cocok dengan admin atau pustakawan, arahkan ke halaman beranda default
...
if (isset($_COOKIE['remember_me'])) {
    ...
    if ($userInfo) {
        ...
        $_SESSION['namaUser'] = $userInfo['namaUser'];
        
        // Pengecekan hak akses dan pengalihan halaman sesuai dengan hak akses
        if ($userInfo['hakAkses'] == "Admin") {
            // Jika pengguna adalah admin, arahkan ke halaman admin
            header("Location: admin/");
            exit();
        } elseif ($userInfo['hakAkses'] == "Pustakawan") {
            // Jika pengguna adalah pustakawan, arahkan ke halaman perpustakaan
            header("Location: perpustakaan/");
            exit();
        } else {
                            ...
                            <div class="card-body">
                                <form method="POST" action="" class="needs-validation" novalidate="">
                                    <div class="form-group">
                                        ...
                                        <input id="username" type="username" autofocus class="form-control" name="username"
