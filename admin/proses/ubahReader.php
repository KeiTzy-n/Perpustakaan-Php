...
if(isset($_GET['id'])) {
    ...
    if($result && $result->num_rows > 0) {
        ...
        $updateQuery = "UPDATE user SET status = '$newStatus' WHERE userID = $userID";
        ...
        if($conn->query($updateQuery)) {
            ...
            echo "<script>
                Swal.fire({
                    icon: 'success',
                    title: 'Sukses',
                    text: 'Status Reader berhasil diperbarui!',
                    showConfirmButton: false,
                    timer: 2000
                }).then(function () {
                    window.location.href = '../reader.php?status=success'; // Redirect setelah menutup SweetAlert
                });
            </script>";
            exit();
        } else {
            ...
            echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Gagal memperbarui status Reader!',
                    showConfirmButton: false,
                    timer: 2000
                }).then(function () {
                    window.location.href = '../reader.php?status=error'; // Redirect setelah menutup SweetAlert
                });
            </script>";
            exit();
        }
        ...
        echo "<script>
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Data Reader tidak ditemukan!',
                showConfirmButton: false,
                timer: 2000
            }).then(function () {
                window.location.href = '../reader.php?status=error'; // Redirect setelah menutup SweetAlert
